...
export default async function handler(req, res) {
  ...
  try {
    // Get all users with digest enabled — no tier filter so early/beta users get it too
    ...
    for (const user of eligible) {
      try {
        ...
        // Generate digest via Claude using real articles
...
Here are today's real headlines to work from:
${articleSummary}

Write a digest using ONLY these real stories. Do not invent headlines. No em dashes, use commas instead.

Return JSON only, no markdown, no backticks:
{"digest":"2-3 sentence overview of today's news","sections":[{"beat":"Beat Name","color":"#c1440e","stories":[{"headline":"real headline from above","summary":"1-2 sentence summary","source":"source name from above","url":"leave blank as #"}]}]}`
            }]
          })
        });
        ...
        // Merge real URLs back in from fetched articles
        const urlMap = {};
        articles.forEach(a => { urlMap[a.headline.slice(0, 60).toLowerCase()] = a.url; });
        parsed.sections.forEach(sec => {
          sec.stories.forEach(s => {
            const key = s.headline.slice(0, 60).toLowerCase();
            s.url = urlMap[key] || articles.find(a =>
              a.source === s.source && a.url
            )?.url || '#';
          });
        });
        ...
        // Send via Resend — single from field, properly formatted
