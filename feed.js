...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Map source name -> beat for tagging
    const _srcBeat = {};
    feeds.forEach(f => { if (f.name && f.beat) _srcBeat[f.name] = f.beat; });

    const results = await Promise.allSettled(
      feeds.map(f => fetchRSS(f.url, f.name).then(articles =>
        articles.map(a => Object.assign({}, a, { _feedBeat: f.beat || null }))
      ))
    );
      ...
      if (r.status === 'fulfilled') {
        r.value.forEach(a => {
          ...
          if (!seen.has(key) && a.headline.length > 10) {
            seen.add(key);
            // Use per-article feed beat (most accurate), fall back to source map, then general
            a.beat = a._feedBeat || _srcBeat[a.source] || 'general';
            delete a._feedBeat;
            articles.push(a);
          }
        });
    ...
    // Sort by recency: parse time strings like "3h ago", "2d ago", "Just now"
    ...
    articles.sort((a, b) => timeToMinutes(a.time) - timeToMinutes(b.time));
